...
class Solution
{
    ...
    void printKDistantRecursive(Node* root, int k, vector<Node*>& v) {
        if (root == NULL) 
            return;
        ...
        if(root->left==NULL && root->right==NULL)
        {
            if(v.size()-1>=k)
            {
                st.insert(v[v.size()-k-1]);
            }
        }
        // Recursive calls for left and right subtrees
        printKDistantRecursive(root->left, k ,v);
        printKDistantRecursive(root->right, k ,v);
        v.pop_back();
    }
    ...
    int printKDistantfromLeaf(Node* root, int k)
    {
        st.clear();
        vector<Node*> v;
        printKDistantRecursive(root,k,v);
        return st.size();
    }
};
