...
class Solution {
  public:
    ...
    Node* preorderBST(int pre[], int size,int& i,int bound)
    {
        if(i>=size || pre[i]>bound) return NULL;
        Node* root=newNode(pre[i++]);
        root->left=preorderBST(pre,size,i,root->data);
        root->right=preorderBST(pre,size,i,bound);
        return root;
    }
    Node* Bst(int pre[], int size) {
       int bound=INT_MAX,i=0;
       return preorderBST(pre,size,i,bound);
    }
};
